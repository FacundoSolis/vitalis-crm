...
let fallos = 0
const exigir = (ok, texto) => { check(ok, texto); if (!ok) fallos++ }
...
console.log('\nRecepción de Madrid (madrid@vitalis.es)')
{
  ...
  exigir(clinicas.length === 1 && clinicas[0] === 'Madrid',
    `solo ve leads de Madrid (${leads.length} leads, clínicas: ${clinicas.join()})`)

  const { error } = await c.from('leads').insert({
    nombre: 'Intruso', telefono: '600000000',
    clinica: 'Sevilla', tratamiento: 'implantes', fuente: 'web',
  })
  exigir(Boolean(error), 'no puede crear un lead en Sevilla (RLS lo bloquea)')

  const { data: propio, error: errorPropio } = await c.from('leads').insert({
    nombre: 'Prueba Madrid', telefono: '+34 600 111 222',
    clinica: 'Madrid', tratamiento: 'ortodoncia', fuente: 'llamada',
  }).select('id, telefono_normalizado').single()
  exigir(!errorPropio, 'sí puede crear un lead en Madrid')
  exigir(propio?.telefono_normalizado === '600111222',
  ...
  const { data: notas } = await c.from('notas').select('texto, tipo').eq('lead_id', propio.id)
  exigir(notas?.some((n) => n.tipo === 'sistema' && n.texto.includes('nuevo → contactado')),
...
console.log('\nGerencia (gerencia@vitalis.es)')
{
  ...
  const { data: dup } = await c.from('leads').select('nombre').eq('telefono_normalizado', '611245890')
  exigir(dup?.length === 2,
